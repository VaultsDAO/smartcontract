...
module.exports = async function (deployer, network) {
  ...
  if (options.migration) {
    ...
    if ((await configProvider.auctionFeePercentage()).toString() != '0') {
      console.log('configProvider.setAuctionFeePercentage')
      await configProvider.setAuctionFeePercentage(0)
    }
    if ((await configProvider.rebuyDuration()).toString() != '0') {
      console.log('configProvider.setRebuyDuration')
      await configProvider.setRebuyDuration(0)
    }
